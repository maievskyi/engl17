...
int main(int argc, const char ** argv, const char** env)
{
 ...
 под ini структуру-настройки программы \n",
	  ...
	  // значит есть открытый fini.dat  ____________________________________________
	...
	if ('y' == getch(stdin))//  ==================================================
	{
		...
		if (err) {
			...
			pmemtxtbuf = (char*)malloc(sizeof(char) * (txtSize)+1); // выделить дин память pmemtxtbuf для хран-я содерж-го из ф-л а
