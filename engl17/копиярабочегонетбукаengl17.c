...
/// далее ОТКР PfINI, СОЗД-СЯ Д ПАМ PMEMINI, ЗАПОЛН ПО УМОЛЧ-Ю И ЗАП В Ф ПОТОК 
 ...
 под ini структуру-настройки программы \n",
	...
	//далее !!!! дальнейшая инициализация=0 + настройки по умолчани
	memset(pmemini, NULL, sizeof(struct inidat)); //    заполн. нулями
	

	err = fopen_s(&pFini, "fini.dat", "w+b");// открывается на чтение и запись
	if (err)	// значит нет fini.dat								
	{
		perror("fini.dat");
...
{		
	...
	err = fopen_s(&pFtxt,TEXTIN, "r+b");// инициал-тся указ=ль FILE *pFtxt 
	...
	//else {	//происходит открытие входного text00.txt файла 
...
// перенос ф-ла текста в оперативную динам память, для цього ---> ================
		...
		if (pmemini == NULL)
		{
				printf("Не выделена память ini настройки программы \n");
		}
		else
		{
...
#ifdef TEXTD	//~~~~~ текст выв-ся в станд. поток вывода консоли НА ЭКРАН (для  отладки)============ =======
...
}	// end "ХОТИТЕ ЛИ ИЗМЕНИТЬ НАСТРОЙКИ УРОКА ? - нажмите 'y'"...............
else
