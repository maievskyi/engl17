...
//// открытие=проверка сущ-вания стар ini файла пользов настр "fini.dat"==================		
	...
	////запись  стандартн по умлч настроек из дин памяти pmeminidat в ф fini.dat  ========		
		...
		{
			puts("\n Переоткрылся на чт с дозап нов ini файл польз-ля с стандартн настр  \n");
		}

		system("pause");
	} // end if "нет  стар fini.dat r+b" = нет. Сделан новый ,,,,,,,,,,,,,,,,,,,,,,,,
	else// Новый INI не нужен
	{
// Новый INI не нужен
...
////  end есть ли fini.dat  === теперь точно есть УЖЕ ПО ЛЮБОМУ открытый fini.dat====== 

	  // значит теперь у нас есть УЖЕ ПО ЛЮБОМУ открытый fini.dat  _______________________________________
	// или тут надо бы секунд на 5 вывести-визуализировать на экран имеющиеся настройки pmemini
